...
public class HiveLegacyFunctionExtractor {
  ...
  public ScalarFunction extract(Function function, Method method) {
    ...
    ScalarType retType;
    try {
      fnArgsList = resolveArgumentTypes(Arrays.asList(fnArgs));
  ...
  protected List<Type> resolveArgumentTypes(List<Class<?>> arguments)
    ...
    for (Class<?> argument : arguments) {
      resolvedTypes.add(resolveArgumentType(argument));
    }
    return resolvedTypes;
  }

  protected Type resolveArgumentType(Class<?> argument) throws ImpalaException {
    JavaUdfDataType javaUdfType = JavaUdfDataType.getType(argument);
    if (javaUdfType == JavaUdfDataType.INVALID_TYPE) {
      throw new CatalogException("Param type " + argument + " not supported.");
    }
  ...
  protected ScalarType resolveReturnType(Class<?> returnType) throws ImpalaException {
    JavaUdfDataType javaRetType = JavaUdfDataType.getType(returnType);
    if (javaRetType == JavaUdfDataType.INVALID_TYPE) {
      throw new CatalogException("Return type " + returnType + " not supported.");
    }
    return ScalarType.createType(
