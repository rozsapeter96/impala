...
public interface TranslationRule {
  ...
  /**
   * Per-key prefix rewrite: for every 'source' key starting with 'sourcePrefix', invokes
   * 'rewriter' with the suffix (the part after the prefix), the value, and 'output'. The
   * rewriter emits one or more output entries (typically via {@link #putChecked}). Used
   * for per-account keys such as Azure 'adls.sas-token.&lt;account&gt;'.
   */
  class PrefixRewriteRule implements TranslationRule {
    /** Receives (suffix, value, output) for each matched key. */
    public interface Rewriter {
      void rewrite(String suffix, String value, Map<String, String> output);
    }

    private final String sourcePrefix;
    private final Rewriter rewriter;

    public PrefixRewriteRule(String sourcePrefix, Rewriter rewriter) {
      this.sourcePrefix = Preconditions.checkNotNull(sourcePrefix);
      this.rewriter = Preconditions.checkNotNull(rewriter);
    }

    @Override
    public boolean apply(Map<String, String> source, Map<String, String> output) {
      boolean applied = false;
      // Collect matches first; we mutate 'source' while iterating.
      ImmutableMap.Builder<String, String> matched = ImmutableMap.builder();
      for (Map.Entry<String, String> e : source.entrySet()) {
        if (e.getKey().startsWith(sourcePrefix)) {
          matched.put(e.getKey(), e.getValue());
        }
      }
      for (Map.Entry<String, String> e : matched.build().entrySet()) {
        source.remove(e.getKey());
        rewriter.rewrite(
            e.getKey().substring(sourcePrefix.length()), e.getValue(), output);
        applied = true;
      }
      return applied;
    }
