...
public class RESTCatalogProperties {
  ...
  public RESTCatalogProperties(Properties properties) {
    ...
    // When vending is enabled, request it via the access-delegation header, and read
    // table metadata through the FileIO that honours the per-table credential. The
    // default FileIO resolves locations through the shared Hadoop FileSystem cache,
    // which ignores the credential on the Configuration - see VendedCredentialsFileIO.
    ...
    if (credentialsEnabled) {
      ...
      finalMap_.put(CatalogProperties.FILE_IO_IMPL,
          VendedCredentialsFileIO.class.getName());
    }
  }
