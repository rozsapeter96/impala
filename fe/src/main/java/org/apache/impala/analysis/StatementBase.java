...
public abstract class StatementBase extends StmtNode {
  ...
  public static Expr checkTypeCompatibility(String dstTableName, Column dstCol,
    ...
    TypeCompatibility compatibility = analyzer.getRegularCompatibilityLevel();

    Type compatType =
        Type.getAssignmentCompatibleType(srcExprType, dstColType, compatibility);
    ...
    if (compatType.isInvalid() && permissiveCompatibility.isUnsafe()) {
      ...
      compatibility = permissiveCompatibility;
    }
    ...
    return srcExpr.castTo(compatType, compatibility);
  }
}
