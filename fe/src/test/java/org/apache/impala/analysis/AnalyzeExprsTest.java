...
public class AnalyzeExprsTest extends AnalyzerTest {
  ...
  public void testUnsafeCasts() {
    ...
    for (String numericType : numericTypes) {
      for (Pair<String, String> stringType : stringTypes) {
        ...
        String nonConstStatement = String.format(
            "insert into functional.alltypesnopart(%s_col) select string_col from functional.alltypes",
            numericType);
    ...
    AnalysisError(
        ...
        unsafeCtx,
        "Target table 'functional.alltypesnopart' is incompatible with source expressions.\n"
            + "Expression 'cast(100 as decimal(9,0))' (type: DECIMAL(9,0))"
            + " is not compatible with column 'string_col' (type: STRING)");
  }
}
