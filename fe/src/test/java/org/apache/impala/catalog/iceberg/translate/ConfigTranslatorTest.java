...
public class ConfigTranslatorTest {
  ...
  public void testVerifyCaseInsensitiveSucceeds() {
    ...
    assertEquals("none", translate(rule, ImmutableMap.of("session", "none")).get("session"));
  }
  ...
  @Test
  public void testPrefixRewritePerAccount() {
    TranslationRule rule = new TranslationRule.PrefixRewriteRule("adls.sas-token.",
        (account, value, output) -> {
          output.put("fs.azure.account.auth.type." + account, "SAS");
          output.put("fs.azure.sas.fixed.token." + account, value);
        });
    Map<String, String> out = translate(rule, ImmutableMap.of("adls.sas-token.acct1", "TOKEN"));
    assertEquals("SAS", out.get("fs.azure.account.auth.type.acct1"));
    assertEquals("TOKEN", out.get("fs.azure.sas.fixed.token.acct1"));
  }
  ...
  public void testPassThroughRemainingFalseDropsUnknownKeys() {
    Map<String, String> out = translate(
        new TranslationRule.RenameRule("uri"), ImmutableMap.of("uri", "x", "unknown", "y"));
    assertEquals("x", out.get("uri"));
