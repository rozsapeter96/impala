...
class TestIcebergV2Table(IcebergTestSuite):
  ...
  def test_drop_partition(self, vector, unique_database):
    create_table = """CREATE TABLE IF NOT EXISTS {}.iceberg_all_partitions
        ...
        unpartitioned_to_identity_int int, unpartitioned_to_bucket_string string,
        unpartitioned_to_month_date date, unpartitioned_after_evolution_string string)
      PARTITIONED BY SPEC
  ...
  def test_rollback_after_drop_partition(self, vector, unique_database):
    ...
    create_table = """CREATE TABLE {}(identity_int int, unpartitioned_int int)
      PARTITIONED BY SPEC (identity_int) STORED AS ICEBERG
      TBLPROPERTIES('format-version'='2')""".format(qualified_table_name)
    insert_into = """INSERT INTO {} values(1, 2)""".format(qualified_table_name)
    drop_partition = """ALTER TABLE {} DROP PARTITION (identity_int = 1)""".format(
      qualified_table_name)

    self.execute_query(create_table)
    self.execute_query(insert_into)
    self.execute_query(drop_partition)
