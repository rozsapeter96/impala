...
///
/// Init() is called once at daemon startup.  Get() returns the resulting
/// HdfsConfigProperties for use as the HdfsFsCache::GetConnection() fallback credential
/// when no per-query vended credential covers the requested path.
class S3ConnCredentials {
 public:
  /// Runs both commands (if set) and stores the resulting fs.s3a.access.key /
  /// fs.s3a.secret.key in global_config_.  No-op when neither flag is set; error if
  /// only one is set or a command fails.
  static Status Init();
  ...
  static const HdfsConfigProperties& Get() { return global_config_; }

 private:
  static HdfsConfigProperties global_config_;
};
